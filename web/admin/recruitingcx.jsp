...
<form name="f1" autocomplete="off"  action="/itivtmgr/recruiting/recruitingcx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
<input type=button   class='' value=' 添加信息 ' onclick="window.location.replace('/itivtmgr/recruiting/torecruitingtj.do')" />&nbsp;&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">
<!----><a href="/itivtmgr/recruiting/topvirecruitingxg.do?spage=recruitingcx&id=${sdata.id}">查看</a>
  |  <a href="/itivtmgr/recruiting/torecruitingxg.do?id=${sdata.id}">修改</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/recruiting/delete.do?plids=${sdata.id}&surl=recruitingcx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
if(countplids()) { 
f1.action='/itivtmgr/recruiting/customUpdate.do?surl=recruitingcx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/recruiting/delete.do?surl=recruitingcx'; 
f1.submit(); 
}
}
}
