...
<form name="f1" autocomplete="off"  action="/itivtmgr/charges/chargescx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
<input type=button   class='' value=' 添加信息 ' onclick="window.location.replace('/itivtmgr/charges/tochargestj.do?topage=chargestj')" />&nbsp;&nbsp;
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">
<!----><a href="/itivtmgr/charges/topvichargesxg.do?spage=chargescx&id=${sdata.id}&topage=pvichargesxg">查看</a>
  |  <a href="/itivtmgr/charges/tochargesxg.do?id=${sdata.id}&topage=chargesxg">修改</a>
						|
						<a href="javascript:add('20000${sdata.id}')">在线交流</a>
 |
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/charges/delete.do?plids=${sdata.id}&surl=chargescx">删除</a>
</td>
</tr>
</c:forEach>
				<tr>
...
if(countplids()) {
f1.action='/itivtmgr/charges/customUpdate.do?surl=chargescx&col='+col+'';
...
if(confirm('是否确认删除？')){
if(countplids()) {
f1.action='/itivtmgr/charges/delete.do?surl=chargescx';
f1.submit();
}
}
}
