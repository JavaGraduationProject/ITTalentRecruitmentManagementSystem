...
<form name="f1" autocomplete="off"  action="/itivtmgr/ucollect/qyucollectcx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
 ...
 企业名称 
: 
<input type=text class=''  size=15 name='ptitle' />
&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">${sdata.ptitle}</td> 
					<td scope="col">
<!----><a href="/itivtmgr/sysuser/toqyxxx.do?id=${sdata.pid}" target="_blank">查看</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/ucollect/delete.do?plids=${sdata.id}&surl=qyucollectcx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
if(countplids()) { 
f1.action='/itivtmgr/ucollect/customUpdate.do?surl=qyucollectcx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/ucollect/delete.do?surl=qyucollectcx'; 
f1.submit(); 
}
}
}
