...
<form name="f1" autocomplete="off"  action="/itivtmgr/ucollect/ucollectcx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
<input type=button   class='' value=' 添加信息 ' onclick="window.location.replace('/itivtmgr/ucollect/toucollecttj.do')" />&nbsp;&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">
<!----><a href="/itivtmgr/ucollect/toucollectxg.do?id=${sdata.id}">修改</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/ucollect/delete.do?plids=${sdata.id}&surl=ucollectcx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
<script language=javascript> 
function update(no){ 
pop('ucollectxg.jsp?id='+no,'信息修改',550,'100%') 
}
</script> 
<script language=javascript> 
function add(){ 
pop('ucollecttj.jsp','信息添加',550,'100%') 
}
</script> 
<script language=javascript> 
function batchUpdate(col,cvalue,id){ 
if(countplids()) { 
f1.action='/itivtmgr/ucollect/customUpdate.do?surl=ucollectcx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/ucollect/delete.do?surl=ucollectcx'; 
f1.submit(); 
}
}
}
