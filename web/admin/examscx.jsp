...
<form name="f1" autocomplete="off"  action="/itivtmgr/exams/examscx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
<input type=button   class='' value=' 添加信息 ' onclick="window.location.replace('/itivtmgr/exams/toexamstj.do?topage=examstj')" />&nbsp;&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
	...
	<td scope="col">${sdata.grade}</td>
	<td scope="col">
<!----><a href="/itivtmgr/exams/topviexamsxg.do?spage=examscx&id=${sdata.id}&topage=pviexamsxg">查看</a>
  |  <a href="/itivtmgr/exams/toexamsxg.do?id=${sdata.id}&topage=examsxg">修改</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/exams/delete.do?plids=${sdata.id}&surl=examscx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
if(countplids()) { 
f1.action='/itivtmgr/exams/customUpdate.do?surl=examscx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/exams/delete.do?surl=examscx'; 
f1.submit(); 
}
}
}
