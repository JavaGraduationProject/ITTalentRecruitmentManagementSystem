...
<!-- servletformstart 
<form  action="/itivtmgr/itivtmgr?ac=zrtypestj&tglparentid=${tglparentid}"   autocomplete="off"  style="display: inline;font-size: 15px" name="f1" method="post"  onsubmit="return checkform()" >
 servletformend -->
<!-- s2formstart -->
<form name="f1" method="post" onsubmit="return checkform()" action="/itivtmgr/zrtypes/zrtypestj.do?tglparentid=${tglparentid}" > 
<!-- servletformend --> 
...
&nbsp;&nbsp;&nbsp;&nbsp;                     
<input type=button value=' 返回上页 ' onclick='window.location.replace("/itivtmgr/zrtypes/zrtypescx.do")' />   
 </td> 
</tr>
...
if(tnameobj.value!=""){  
var ajax = new AJAX();
ajax.post(encodeURI("/itivtmgr/ajax/checkno.do?table=zrtypes&col=tname&value="+tnameobj.value+"&checktype=insert&ttime=<%=Info.getDateStr()%>")) 
//ajax.post("/itivtmgr/ajax/checkno.do?table=zrtypes&col=tname&value="+tnameobj.value+"&checktype=insert&ttime=<%=Info.getDateStr()%>") 
