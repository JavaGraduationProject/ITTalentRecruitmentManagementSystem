...
<!-- servletformstart 
<form  action="/itivtmgr/itivtmgr?ac=zrtypesxg&id=${id}"   autocomplete="off"  style="display: inline;font-size: 15px" name="f1" method="post"  onsubmit="return checkform()" >
 servletformend -->
<!-- s2formstart -->
<form  name="f1" method="post"  onsubmit="return checkform()" action="/itivtmgr/zrtypes/zrtypesxg.do?id=${id}" > 
<!-- servletformend --> 
...
if(tnameobj.value!=""){  
var ajax = new AJAX(); 
ajax.post("/itivtmgr/ajax/checkno.do?table=zrtypes&col=tname&value="+tnameobj.value+"&checktype=update&id=${id}&ttime=<%=Info.getDateStr()%>"); 
//ajax.post(encodeURI("/itivtmgr/ajax/checkno.do?table=zrtypes&col=tname&value="+tnameobj.value+"&checktype=update&id=${id}&ttime=<%=Info.getDateStr()%>")); 
