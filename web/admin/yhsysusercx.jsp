...
<form name="f1" autocomplete="off"  action="/itivtmgr/sysuser/yhsysusercx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
<input type=button   class='' value=' 添加信息 ' onclick="window.location.replace('/itivtmgr/sysuser/toyhsysusertj.do')" />&nbsp;&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">
<!--
<a href="/itivtmgr/sysuser/customUpdate.do?surl=yhsysusercx&status=锁定&id=${sdata.id}">锁定</a>
 | 
<a href="/itivtmgr/sysuser/customUpdate.do?surl=yhsysusercx&status=正常&id=${sdata.id}">解锁</a>
 | 
--><a href="/itivtmgr/sysuser/toyhsysuserxg.do?id=${sdata.id}">修改</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/sysuser/delete.do?plids=${sdata.id}&surl=yhsysusercx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
<script language=javascript> 
function update(no){ 
pop('yhsysuserxg.jsp?id='+no,'信息修改',550,'100%') 
}
</script> 
<script language=javascript> 
function add(){ 
pop('yhsysusertj.jsp','信息添加',550,'100%') 
}
</script> 
<script language=javascript> 
function batchUpdate(col,cvalue,id){ 
if(countplids()) { 
f1.action='/itivtmgr/sysuser/customUpdate.do?surl=yhsysusercx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/sysuser/delete.do?surl=yhsysusercx'; 
f1.submit(); 
}
}
}
