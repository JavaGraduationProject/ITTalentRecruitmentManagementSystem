...
<!-- servletformstart 
<form  action="/itivtmgr/itivtmgr?ac=postypetj&tglparentid=${tglparentid}"   autocomplete="off"  style="display: inline;font-size: 15px" name="f1" method="post"  onsubmit="return checkform()" >
 servletformend -->
<!-- s2formstart -->
<form name="f1" method="post" onsubmit="return checkform()" action="/itivtmgr/postype/postypetj.do?tglparentid=${tglparentid}" > 
<!-- servletformend --> 
...
if(datashownameobj.value!=""){  
var ajax = new AJAX();
ajax.post(encodeURI("/itivtmgr/ajax/checkno.do?table=postype&col=datashowname&value="+datashownameobj.value+"&checktype=insert&ttime=<%=Info.getDateStr()%>")) 
//ajax.post("/itivtmgr/ajax/checkno.do?table=postype&col=datashowname&value="+datashownameobj.value+"&checktype=insert&ttime=<%=Info.getDateStr()%>") 
