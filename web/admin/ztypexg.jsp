...
<!-- servletformstart 
<form  action="/itivtmgr/itivtmgr?ac=ztypexg&id=${id}"   autocomplete="off"  style="display: inline;font-size: 15px" name="f1" method="post"  onsubmit="return checkform()" >
 servletformend -->
<!-- s2formstart -->
<form  name="f1" method="post"  onsubmit="return checkform()" action="/itivtmgr/ztype/ztypexg.do?id=${id}" > 
<!-- servletformend --> 
...
if(znameobj.value!=""){  
var ajax = new AJAX(); 
ajax.post("/itivtmgr/ajax/checkno.do?table=ztype&col=zname&value="+znameobj.value+"&checktype=update&id=${id}&ttime=<%=Info.getDateStr()%>"); 
//ajax.post(encodeURI("/itivtmgr/ajax/checkno.do?table=ztype&col=zname&value="+znameobj.value+"&checktype=update&id=${id}&ttime=<%=Info.getDateStr()%>")); 
