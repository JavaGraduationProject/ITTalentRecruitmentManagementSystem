...
<form name="f1" autocomplete="off"  action="/itivtmgr/zchats/zchatscx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
<input type=button   class='' value=' 发起会话 ' onclick="window.location.replace('/itivtmgr/zchats/tozchatstj.do')" />&nbsp;&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">${sdata.tcontent}</td> 
					<td scope="col">${sdata.savetime}</td> 
					<td scope="col">
<a href="javascript:add('${sdata.id}')">在线交流</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/zchats/delete.do?plids=${sdata.id}&surl=zchatscx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
<script language=javascript> 
function update(no){ 
pop('zchatsxg.jsp?id='+no,'信息进入会话',550,'100%') 
}
</script> 
<script language=javascript> 
function add(){ 
pop('zchatstj.jsp','信息添加',550,'100%') 
}
</script> 
<script language=javascript> 
function batchUpdate(col,cvalue,id){ 
if(countplids()) { 
f1.action='/itivtmgr/zchats/customUpdate.do?surl=zchatscx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/zchats/delete.do?surl=zchatscx'; 
f1.submit(); 
}
}
}
