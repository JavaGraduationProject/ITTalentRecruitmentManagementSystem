...
<form name="f1" autocomplete="off"  action="/itivtmgr/sysuser/qysysusercx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
<input type=button   class='' value=' 添加信息 ' onclick="window.location.replace('/itivtmgr/sysuser/toqysysusertj.do')" />&nbsp;&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">
<!--
<a href="/itivtmgr/sysuser/customUpdate.do?surl=qysysusercx&status=锁定&id=${sdata.id}">锁定</a>
 | 
<a href="/itivtmgr/sysuser/customUpdate.do?surl=qysysusercx&status=正常&id=${sdata.id}">解锁</a>
 | 
--><a href="/itivtmgr/sysuser/toqysysuserxg.do?id=${sdata.id}">修改</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/sysuser/delete.do?plids=${sdata.id}&surl=qysysusercx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
<script language=javascript> 
function add(){ 
pop('qysysusertj.jsp','信息添加',550,'100%') 
}
</script> 
<script language=javascript> 
function batchUpdate(col,cvalue,id){ 
if(countplids()) { 
f1.action='/itivtmgr/sysuser/customUpdate.do?surl=qysysusercx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/sysuser/delete.do?surl=qysysusercx'; 
f1.submit(); 
}
}
}
