...
<form name="f1" autocomplete="off"  action="/itivtmgr/messages/messagescx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
&nbsp; 
 ...
 内容 
: 
<input type=text class=''  size=15 name='cont' />
&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">${sdata.cont}</td> 
					<td scope="col">${sdata.recont}</td> 
					<td scope="col">${sdata.savetime}</td> 
					<td scope="col">
<!----><a href="/itivtmgr/messages/tomessagesxg.do?id=${sdata.id}">回复</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/messages/delete.do?plids=${sdata.id}&surl=messagescx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
if(countplids()) { 
f1.action='/itivtmgr/messages/customUpdate.do?surl=messagescx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/messages/delete.do?surl=messagescx'; 
f1.submit(); 
}
}
}
