...
<form name="f1" autocomplete="off"  action="/itivtmgr/sysuser/hysysusercx.do"  method="post" style="display: inline;font-size: 14px">
<div class="ms-panel">
...
<input type=button   class='' value=' 添加信息 ' onclick="window.location.replace('/itivtmgr/sysuser/tohysysusertj.do')" />&nbsp;&nbsp; 
...
<input type='checkbox' value='${sdata.id}' name='plids' />
					...
					<td scope="col">
<!--
<a href="/itivtmgr/sysuser/customUpdate.do?surl=hysysusercx&status=锁定&id=${sdata.id}">锁定</a>
 | 
<a href="/itivtmgr/sysuser/customUpdate.do?surl=hysysusercx&status=正常&id=${sdata.id}">解锁</a>
 | 
--><a href="/itivtmgr/sysuser/tohysysuserxg.do?id=${sdata.id}">修改</a>
 | 
<a onclick="return confirm('确定要删除这条记录吗?')" href="/itivtmgr/sysuser/delete.do?plids=${sdata.id}&surl=hysysusercx">删除</a>
</td>
</tr>
</c:forEach> 
				<tr> 
...
if(countplids()) { 
f1.action='/itivtmgr/sysuser/customUpdate.do?surl=hysysusercx&col='+col+''; 
...
if(confirm('是否确认删除？')){ 
if(countplids()) { 
f1.action='/itivtmgr/sysuser/delete.do?surl=hysysusercx'; 
f1.submit(); 
}
}
}
