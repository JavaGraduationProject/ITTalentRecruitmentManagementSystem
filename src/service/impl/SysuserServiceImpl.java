...
public class SysuserServiceImpl implements SysuserService {
	...
	{
		List<HashMap> list = PageManager.getPages(pageUrl,pageSize, SysuserMapper.class.getName()+"."+selectMethod,selectmap, request ); 
		if(!"Sysuser".equals("Sysuser"))
	...
	{
		mapper.update(map);
		new CommDAO().updateById("exams","qyuname","sysuser","uname~hnoname","","1" );
		new CommDAO().updateById("exams","touname","sysuser","uname~hnoname~hnoname","","0" );
		new CommDAO().updateById("charges","uname","sysuser","uname~hnoname","","1" );
